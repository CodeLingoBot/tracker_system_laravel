@extends('layouts.app')
@section('title', __('roles.'.($role->id?'edit':'new').'_title'))
...
@section('layout-content')
    <form class="" method="POST" action="{{ $role->id ? route('roles.update', $role) : route('roles.store') }}"
          style="width: 100%;">
        {{ csrf_field() }}
        ...
        <div class="form-group{{ $errors->has('level') ? ' has-error' : '' }}">
            ...
            <input id="level" type="number" class="form-control" name="level" value="{{ old('level', $role->level) }}"
                   required autofocus>
        ...
        <div class="form-group{{ $errors->has('slug') ? ' has-error' : '' }}">
            ...
            <input id="slug" type="text" class="form-control" name="slug" value="{{ old('slug', $role->slug) }}"
                   required autofocus>
        ...
        <div class="form-group{{ $errors->has('description') ? ' has-error' : '' }}">
            ...
            <input id="description" type="text" class="form-control" name="description"
                   value="{{ old('description', $role->description) }}" required autofocus>
